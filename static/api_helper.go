...
func PostRequest[T any](url, token string, data interface{}, t *T, params ...string) (int, *response[T], error) {
	c := fiber.Client{}
	a := c.Post(url)
...
func GetRequest[T any](url, token string, data interface{}, t *T, params ...string) (int, *response[T], error) {
	c := fiber.Client{}
	a := c.Get(url)
	a.Add("Authorization", "Bearer "+token)
